...
| **Vercel** | [Project Settings → Git](https://vercel.com/docs/project-configuration/git-settings)에서 **Git Large File Storage (LFS)** 를 **켜야** 클론 시 실파일이 내려온다. 끄면 포인터만 있어 런타임 JSON 오류가 난다. 빌드 커맨드에 `git lfs pull` 을 넣지 말 것 — Vercel 환경에서 `missing protocol: ""` 로 실패할 수 있다. `ROUTER_FILESYSTEM_LOOKUP_ERROR` 등 라우터 오류는 [`docs/VERCEL-TROUBLESHOOTING.md`](./VERCEL-TROUBLESHOOTING.md) 참고. |
