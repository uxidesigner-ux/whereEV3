...
export function BootEvCarAnimation({ reduceMotion = false }) {
  ...
  /** 라이트: 화이트 보디 + 스트로크·이중 그림자로 실루엣 분리 / 다크: 밝은 톤 + 블루 글로우 */
  const bodyShadow = isDark
    ? 'drop-shadow(0 4px 14px rgba(37, 99, 235, 0.35))'
    : 'drop-shadow(0 2px 12px rgba(15, 23, 42, 0.28)) drop-shadow(0 1px 3px rgba(15, 23, 42, 0.18))'
  const carFill = isDark ? theme.palette.grey[100] : '#ffffff'
  const bodyStroke = isDark ? 'rgba(0, 0, 0, 0.22)' : 'rgba(15, 23, 42, 0.24)'
  const strokeW = isDark ? 0.45 : 0.9
    ...
    >
      ...
      >
        ...
        >
          ...
          />
          {!isDark ? (
            <ellipse cx="27" cy="38" rx="3.2" ry="2.4" fill="rgba(59,130,246,0.35)" opacity={0.9} />
          ) : null}
          {!isDark ? (
            <ellipse cx="45" cy="38" rx="3.2" ry="2.4" fill="rgba(59,130,246,0.35)" opacity={0.9} />
          ) : null}
        </Box>
      </Box>
    </Box>
